            ...
            <div class="card card-primary">
                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table table-sm table-hover" id="category-table">
...
<script>
    $(document).ready(function () {
        ...
        $('#category-table').DataTable({
            ...
            buttons: [{
                extend: 'print',
                title: 'LMS - Categories (' + date + ')',
                filename: 'LMS - Categories (' + date + ')',
                exportOptions: {
                    columns: [1, 2]
                }
            },
        ...
        $('#category-table').DataTable().on('draw', function () {
        ...
        $('#btn-add').click(function () {
            ...
            $('#category-form').find('.form-control').removeClass('is-invalid is-valid');
        ...
        $('body').on('click', '#btn-edit', function () {
            ...
            $.get("{{ route('admin.categories.index') }}" + '/' + id + '/edit', function (data) {
                $('#category-form').find('.form-control').removeClass('is-invalid is-valid');
                $('#id').val(data.id);
                $('#name').val(data.name);
        ...
        // Store new Category or update Category
        $('#btn-save').click(function () {
            ...
            var type = "POST";
            var ajaxurl = "{{ route('admin.categories.store') }}";
            ...
            if (state == "update") {
                ...
                var id = $('#id').val();
                type = "PUT";
                ajaxurl = "{{ route('admin.categories.store') }}" + '/' + id;
            }
            ...
            $.ajax({
                ...
                success: function (data) {
                    if (state == "save") {
                        ...
                        $('#category-table').DataTable().draw(false);
                        $('#category-table').DataTable().on('draw', function () {
                            $('[data-toggle="tooltip"]').tooltip();
                        });
                    } else {
            ...
            swal("Whoops!", "Are you sure want to delete? \n (name : " + name + ")", "warning", {
                ...
                switch (value) {
                    case "ok":
                        $.ajax({
                            type: "DELETE",
                            url: "{{ route('admin.categories.store') }}" + '/' + id,
                            success: function (data) {
                                $('#category-table').DataTable().draw(false);
                                $('#category-table').DataTable().on('draw',
                                    function () {
                                        $('[data-toggle="tooltip"]').tooltip();
                                    });
                ...
                swal("Whoops!", "Are you sure want to delete these selected rows?", "warning", {
                    ...
                    switch (value) {
                        case "ok":
                            $.ajax({
                                type: "POST",
                                url: "{{ route('admin.categories.deleteAllSelected') }}",
                                data: {
                                    ids: ids,
                                },
                                success: function (data) {
                                    ...
                                    swal({
                                        title: "Well Done!",
                                        text: "Selected categories were successfully deleted",
                                        icon: "success",
                                        timer: 3000
                                    });
                                },

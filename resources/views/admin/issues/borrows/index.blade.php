        ...
        <div class="section-body">
            <div class="card card-primary">
                ...
                <div class="card-header">
                    <a class="btn btn-primary" href="{{ route('admin.issues.borrows.create') }}">
                        ...
                        <div class="row">
                            <div class="col-md-3 col-sm-5">
                                <select class="select2 form-control" id="filter">
                                    ...
                                    <option value="{{ $date['borrow_date'] }}">
                                        {{ date('d/m/Y', strtotime($date['borrow_date'])) }}
                                    </option>
                                    @endforeach
                                </select>
                            </div>
    ...
    $(document).ready(function() {
            ...
            $.ajax({
                ...
                success: function(res) {
                    ...
                    if (res.data.year) {
                        if ($('#year_parent').length === 0) {
                            $('.user-stats').find('#stocks_parent').before(
                                ...
                                res.data.year +
                                '</div><div class="title">Year</div></div>');
                        }
                    } else {
                    ...
                    if (res.data.pages) {
                        if ($('#pages_parent').length === 0) {
                            $('.user-stats').find('#stocks_parent').before(
                                ...
                                res.data.pages +
                                '</div><div class="title">Pages</div></div>');
                        }
                    } else {
                    ...
                    if (res.data.ebook_available === '0') {
                        ...
                        var url = "{{ url('') }}";
                        $('#available p').html('Yes, <a target="_blank" href="' + url +
                            '/pdfs/' + res.data.ebook_url +
                            '"><i class="fas fa-external-link-alt"></i> Read PDF</a>');
                    }
                    ...
                    if (res.data.rack_id !== null) {
                        $('#rack p').html(res.data.rack.position + ' - ' + res.data.rack
                            .category.name);
                    } else {
        ...
        $('body').on('click', '#btn-user', function() {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (data.disabled === '0') {
                        $('#user-modal #status').removeClass('badge-danger').addClass(
                            'badge-success').html('Active');
                    } else {
                        $('#user-modal #status').removeClass('badge-success').addClass(
                            'badge-danger').html('Inactive');
                    }
        ...
        $('#btn-filter').click(function() {
            ...
            $('#issue-table').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: "{{ route('admin.issues.borrows.index') }}",
                    type: 'GET',
                    data: {
                        filter: $('#filter').val()
                    },
                ...
                buttons: [{
                        extend: 'print',
                        title: 'LMS - Borrow (' + date + ')',
                        filename: 'LMS - Borrow (' + date + ')',
                        exportOptions: {
            ...
            $('#issue-table').DataTable().on('draw', function() {
        ...
        $('#issue-table').DataTable({
            processing: true,
            serverSide: true,
            ajax: "{{ route('admin.issues.borrows.index') }}",
            columns: [{
                    data: 'user',
                    name: 'user'
                },
        ...
        $('body').on('click', '#btn-renew', function() {
            ...
            var user = $(this).parents('tr').find('#btn-user').data('user');

            swal("Whoops!", "Renew this book? \n (user : " + user + ") \n (book : " + title + ")",
                "warning", {
                    buttons: {
                ...
                switch (value) {
                    case "ok":
                        $.ajax({
                            type: "PUT",
                            url: "{{ route('admin.issues.borrows.store') }}" + '/' + id + '/renew',
                            data: {
                            ...
                            success: function(data) {
                                ...
                                $('#issue-table').DataTable().draw(false);
                                $('#issue-table').DataTable().on('draw',
                                    function() {
                                        $('[data-toggle="tooltip"]').tooltip();
                                    });
            ...
            swal("Whoops!", "Return this book? \n (user : " + user + ") \n (book : " + title + ")",
                ...
                switch (value) {
                    case "ok":
                        $.ajax({
                            type: "PUT",
                            url: "{{ route('admin.issues.borrows.store') }}" + '/' + id + '/return',
                            data: {
                                issue_id: $(this).data('issue_id'),
                            },
                            success: function(data) {
        ...
        // Lost Book
        $('body').on('click', '#btn-lost', function() {
            var id = $(this).data('book_id');
            var user = $(this).parents('tr').find('#btn-user').data('user');
            var title = $(this).data('book_title');

            swal("Whoops!", "Is this book lost? \n (user : " + user + ") \n (book : " + title + ")",
                "warning", {
                    buttons: {
                        cancel: "No, this book is safe",
                        ok: {
                            text: "Yes, it's lost!",
                            value: "ok"
                        }
                ...
                switch (value) {
                    case "ok":
                        $.ajax({
                            type: "PUT",
                            url: "{{ route('admin.issues.borrows.store') }}" + '/' + id + '/lost',
                            data: {
                                issue_id: $(this).data('issue_id'),
                            },
                            success: function(data) {
